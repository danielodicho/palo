...
function SeoOptimizerTab() {
  ...
  const fetchWebsiteContent = async (url) => {
    try {
      ...
      const response = await fetch(`http://localhost:3010/api/fetch-website?url=${encodeURIComponent(formattedUrl)}`);
...
Only return the JSON — no extra explanation, markdown, or commentary.`;
      ...
      const response = await fetch('http://localhost:3010/api/openai', {
        method: 'POST',
        headers: {
